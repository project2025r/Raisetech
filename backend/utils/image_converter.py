...
def convert_avif_to_jpg_base64(avif_data_url):
    ...
    try:
        ...
        # Read AVIF image using imageio
        img_array = imageio.imread(io.BytesIO(avif_bytes))
        logger.info(f"Successfully read AVIF image with shape: {img_array.shape}")
...
def convert_image_to_yolo_supported(image_data):
    ...
    Returns:
        str: Base64 data URL of JPG image
    """
    try:
        if isinstance(image_data, str):
            # Check if it's already a JPG data URL
            if image_data.startswith('data:image/jpeg;base64,'):
                logger.info("Image is already in JPG format")
                return image_data
            ...
            # For other formats, assume they're already supported
            logger.info("Image format appears to be supported by YOLO")
            return image_data
            
        else:
            # Handle bytes input
            if is_avif_image(image_data):
                logger.info("Converting AVIF bytes to JPG")
                jpg_bytes = convert_avif_to_jpg_bytes(image_data)
                jpg_base64 = base64.b64encode(jpg_bytes).decode('utf-8')
                return f"data:image/jpeg;base64,{jpg_base64}"
            
            # For other formats, convert to base64
            jpg_base64 = base64.b64encode(image_data).decode('utf-8')
            return f"data:image/jpeg;base64,{jpg_base64}"
